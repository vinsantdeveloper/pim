...
public class NativeToJsMessageQueue {
    private static final String TAG = "wpdating"+"JsMessageQueue";
    ...
    public void setBridgeMode(int value) {
        if (value < -1 || value >= bridgeModes.size()) {
            Log.d(TAG, "Invalid NativeToJsBridgeMode: " + value);
        } else {
            BridgeMode newMode = value < 0 ? null : bridgeModes.get(value);
            if (newMode != activeBridgeMode) {
                Log.d(TAG, "Set native->JS mode to " + (newMode == null ? "null" : newMode.getClass().getSimpleName()));
                synchronized (this) {
    ...
    public void addPluginResult(PluginResult result, String callbackId) {
        if (callbackId == null) {
            Log.e(TAG, "Got plugin result with no callbackId", new Throwable());
            return;
        }
    ...
    private void enqueueMessage(JsMessage message) {
        synchronized (this) {
            if (activeBridgeMode == null) {
                Log.d(TAG, "Dropping Native->JS message due to disabled bridge");
                return;
            }
            queue.add(message);
            if (!paused) {
    ...
    public void setPaused(boolean value) {
        if (paused && value) {
            ...
            Log.e(TAG, "nested call to setPaused detected.", new Throwable());
        }
        paused = value;
        if (!value) {
            synchronized (this) {
