...
public class CordovaBridge {
    private static final String TAG ="wpdating"+ "CordovaBridge";
    ...
    private boolean verifySecret(String action, int bridgeSecret) throws IllegalAccessException {
        if (!jsMessageQueue.isBridgeEnabled()) {
            if (bridgeSecret == -1) {
                Log.d(TAG, action + " call made before bridge was enabled.");
            } else {
                Log.d(TAG, "Ignoring " + action + " from previous page load.");
            }
            return false;
        }
        // Bridge secret wrong and bridge not due to it being from the previous page.
        if (expectedBridgeSecret < 0 || bridgeSecret != expectedBridgeSecret) {
            Log.e(TAG, "Bridge access attempt with wrong secret token, possibly from malicious code. Disabling exec() bridge!");
            clearBridgeSecret();
    ...
    public String promptOnJsPrompt(String origin, String message, String defaultValue) {
        ...
        // Polling for JavaScript messages
            ...
            if (pluginManager.shouldAllowBridgeAccess(origin)) {
                ...
                Log.e(TAG, "gap_init called from restricted origin: " + origin);
            }
            return "";
        }
        return null;
